import mongoose, { type Model, type Connection, type ConnectOptions, Schema } from "mongoose";
import _dotenv from 'dotenv';

type CachedConnection = { conn: Connection | null; promise: Promise<Connection> | null };
const connections_cached = new Map<string, CachedConnection>();

const getDbServer =  async function(uri: string, opts?: ConnectOptions){
    ...
    if(!isPresent){
        ...
        const new_connection: CachedConnection = {conn: null, promise: req_connection};
        ...
        const fetched_connection: CachedConnection = connections_cached.get(uri)!;
...
const getDb = async function(conn: Connection, db_name: string){
